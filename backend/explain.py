...
# 2023 / 2024 standard deduction amounts, used only as a rough sanity check —
# not a source of truth for any particular tax year.
...
def build_flags(values: dict[str, float], filing_status: str | None) -> list[Flag]:
    flags: list[Flag] = []
    ...
    if filing_status and deduction is not None:
        lo, hi = _STANDARD_DEDUCTIONS.get(filing_status, (None, None))
        if lo is not None and deduction not in (0,) and abs(deduction - lo) > 100 and abs(deduction - hi) > 100:
            flags.append(Flag("info", f"Deduction on line 12 (${deduction:,.0f}) doesn't match the usual "
                                       f"standard deduction for your filing status (~${lo:,.0f}-${hi:,.0f}), "
                                       "which suggests itemized deductions (Schedule A) were used instead."))
